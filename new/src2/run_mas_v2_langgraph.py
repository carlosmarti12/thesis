...
def build_mas_graph(llm, embedder, candidate_pool):
    ...
    def llm_ranking_agent(state: SynonymState):
        ...
        # First use the LLM ordering, but only for valid original candidates.
        ...
        # Then append all candidates omitted by the LLM.
        # This prevents the LLM ranker from destroying recall.
        for candidate in candidates:
            norm_candidate = normalize(candidate)
        ...
        return {
            "final_candidates": final_order[:5]
        }
...
def run(limit, model_name):
    ...
    run_id = f"mas_v2_llm_ranker_{safe_model_name}_{limit_label}"
