...
def embedding_retrieval_baseline(
    ...
    """
    Closed-world output retrieval baseline:
    ...
    Embeddings are normalized, so the dot product is equivalent to cosine
    similarity — no re-encoding of the candidate pool happens per row.
    """
...
def run_experiment(method: str, limit: int | None, model_name: str):
    ...
    run_id_label = "embedding_output_index" if method == "embedding" else method

    safe_model_name = model_name.replace(":", "_").replace("/", "_")
    limit_label = f"limit_{limit}" if limit is not None else "full"

    if method == "llm":
        run_id = f"{run_id_label}_{safe_model_name}_{limit_label}"
    else:
        run_id = f"{run_id_label}_{limit_label}"
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        try:
            if method == "same_term":
                ...
                candidates = embedding_retrieval_baseline(
                    term, embedder, index_terms, index_embeddings, top_k=5
                )
