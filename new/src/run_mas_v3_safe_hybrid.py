...
def build_mas_graph(llm, embedder, candidate_pool):
    ...
    def generator_agent(state: SynonymState):
        ...
        messages = [
            SystemMessage(
                content=(
                    ...
                    "Return only a JSON list of 5 candidates. "
                    "Do not explain."
                )
            ),
        ...
        return {
            "generated_candidates": candidates[:5]
        }
    ...
    def merge_agent(state: SynonymState):
        ...
        return {
            "merged_candidates": merged[:20]
        }

    def llm_ranking_agent(state: SynonymState):
        term = state["term"]
        candidates = state.get("merged_candidates", [])

        if not candidates:
            return {"llm_ranked_candidates": []}
        ...
        messages = [
            SystemMessage(
                content=(
                    "You are a semantic ranking assistant for domain terminology. "
                    "Order the provided candidates from best synonym to worst synonym for the input term. "
                    "Do not invent new candidates. "
                    "Use only exact strings from the candidate list. "
                    "Return only a JSON list of candidate strings."
                )
            ),
            HumanMessage(
                content=(
        ...
        try:
            response = llm.invoke(messages)
            parsed = parse_candidates(response.content)
        except Exception:
            parsed = []
        ...
        for item in parsed:
            ...
            if norm_item in original_by_norm and norm_item not in seen:
                valid.append(original_by_norm[norm_item])
                seen.add(norm_item)

        return {"llm_ranked_candidates": valid}

    def safe_finalizer_agent(state: SynonymState):
        term = state["term"]
        candidates = state.get("merged_candidates", [])
        llm_ranked = state.get("llm_ranked_candidates", [])
        ...
        llm_position = {
            normalize(candidate): rank
            for rank, candidate in enumerate(llm_ranked)
        }
        ...
        for candidate, emb_score in zip(candidates, embedding_scores):
            ...
            if norm_candidate in llm_position:
                rank = llm_position[norm_candidate]
                llm_bonus = 1.0 / (rank + 1)
            else:
                llm_bonus = 0.0

            # Embedding similarity remains the dominant signal (90%); the LLM
            # ranking only contributes a small bonus (10%) and never penalizes
            # a candidate it omitted — this keeps recall safe from an
            # unreliable local LLM ranker.
    ...
    graph.add_node("llm_ranking_agent", llm_ranking_agent)
    graph.add_node("safe_finalizer_agent", safe_finalizer_agent)
...
def run(limit, model_name):
    ...
    run_id = f"mas_v3_safe_hybrid_{safe_model_name}_{limit_label}"
